# suitHab_bioletters_static_vNext.jl
#
# STATIC, SYNTHETIC, SHORT-PAPER-ORIENTED PIPELINE
# Core point (biotic focus, geometry as context):
#   Area-only forecasts (SAR) miss losses once "trophically supported habitat" becomes fragmented.
#   A simple support-aware "effective-area SAR" (using supported area or supported-LCC area) clarifies mechanism
#   and often reduces the mismatch (except under environmentally biased loss like "front").
#
# Minimal conceptual changes vs your previous script:
#   1) Absolute richness curves (not only incremental losses) to avoid baseline artifacts.
#   2) Bridge test: SAR_eff using effective supported area (total supported footprint and supported LCC area).
#   3) Heatmaps redesigned for smooth interpretability:
#        - denser grid over (k_prey × select_sigma)
#        - metrics: f_collapse (supported-LCC crosses threshold) + peak(SAR_raw error)
#        - "light" runner for sweeps (no per-species patch BFS) to keep it fast and smooth.
#   4) OPTIONAL movement sensitivity sanity check:
#        - patch connectivity radius r in the patch criterion (r=0 vs r=2), but NOT framed as novelty.
#        - kept as a small, low-reps add-on figure (you can disable).
#
# Outputs: displays figures, does NOT save.
#
# Run:
#   JULIA_NUM_THREADS=12 julia suitHab_bioletters_static_vNext.jl
...
# ============================================================
# USER KNOBS (edit these first)
# ============================================================
const DO_MOVEMENT_SANITY = true     # keep sanity check but don't oversell it
const MOVEMENT_R         = 2        # connectivity radius for sanity check
const MOVEMENT_REPS      = 10       # keep low; it's just a check
const MOVEMENT_GEOMS     = [:random, :cluster]  # sanity only

# Main run (full: includes per-species patch BFS for r=0)
const MAIN_SEED   = 1234
const MAIN_REPS   = 35
const N1_MAIN     = 120
const N2_MAIN     = 120
const S_MAIN      = 250

# Sweep (light: no per-species patch BFS) — designed to look smooth and interpretable
const SWEEP_SEED  = 777
const SWEEP_REPS  = 35
const N1_SWEEP    = 100
const N2_SWEEP    = 100
const S_SWEEP     = 180
...
# Heatmap grid (denser + smooth transitions)
const K_LIST      = collect(1:12)
const SEL_LIST    = [0.10, 0.14, 0.20, 0.28, 0.40, 0.55, 0.75, 1.00, 1.35, 1.80]

# Supported-LCC "collapse" threshold (interpretable knob)
const LCC_THRESH  = 0.5

# ============================================================
# Utilities: deterministic per-replicate RNG (thread-schedule independent)
# ============================================================
...
@inline function setting_rng(master_seed::Int, rep::Int, a::Int, b::Int)
    s = splitmix64(UInt64(master_seed) ⊻ (UInt64(rep) * 0x9e3779b97f4a7c15) ⊻ (UInt64(a) << 32) ⊻ UInt64(b))
    return MersenneTwister(Int(mod(s, UInt64(typemax(Int)))))
end

# ============================================================
# Grid indexing + coords
# ============================================================
...
# ============================================================
# Neighbors for r=0 (4-neighborhood), and offsets for radius-r connectivity
# ============================================================
...
function radius_offsets(r::Int)
    ...
    for dr in -r:r, dc in -r:r
        (dr == 0 && dc == 0) && continue
        # Chebyshev radius (square neighborhood) — interpretable "gap crossing"
...
# ============================================================
# Stamp-based BFS LCC (r=0 fast) and LCC with radius connectivity (r>0)
# ============================================================
...
# ============================================================
# Metaweb (TL fixed per rep; diets vary with k_prey, select_sigma)
#   match_sigma: consumer↔prey similarity bias (smaller => more matched)
#   select_sigma: prey guild cohesion (smaller => more "synchronous"/similar prey set)
# ============================================================
...
# ============================================================
# AB computation under fixed prey rule: ">= 1 prey present in cell" (recursive)
...
# ============================================================
# Patch sizes per species (max connected component) under A-only and AB
# Uses fast shortcut: if total count < Epatch => maxpatch=count (no BFS).
# Connectivity: r=0 uses 4-neigh; r>0 uses radius offsets (sanity only).
# ============================================================
...
# ============================================================
# SAR fits on A-only (raw area), using total and patch presence definitions
# ============================================================
function sar_fit_power(xs::Vector{Float64}, ys::Vector{Float64})
    # fits log(ys) = b + z log(xs); returns (c, z, r2)
    x = Float64[]; y = Float64[]
...
# ============================================================
# Bridge fits: SAR_eff using effective supported area from AB
#   - total-area version: richness vs total supported footprint fraction
#   - patch version:      richness vs supported LCC area fraction
# This is a *mechanistic correction test*, not a claim of new "SAR theory".
# ============================================================
function sar_fit_eff_AB(rng::AbstractRNG, A::BitMatrix, TL::Vector{Int}, preylist::Vector{Vector{Int}};
    ...
    for a in area_fracs
        ...
        for s in 1:samples
            ...
            # AB richness under total-area
            S_AB_tot = count(>=(Emin_total), ABcount)

            # Effective supported area (consumer footprint)
            ...
            # AB richness under patch criterion r=0 only (consistent with main)
            ...
            # Effective LCC area of supported footprint
            if footprint > 0
...
# ============================================================
# Configuration + thread-local buffers
# ============================================================
...
# ============================================================
# One replicate: generate env/A/TL once; diets depend on (k_prey, select_sigma)
# ============================================================
function run_one_replicate!(cfg::RunConfiguration, rng::AbstractRNG,
    ...
    # Trophic levels fixed per rep
    TL, basals, _isbasal = make_trophic_levels(rng; S=S, basal_frac=cfg.basal_frac, Lmax=cfg.Lmax)

    # Diets vary with cfg.k_prey and cfg.select_sigma (still fixed within rep for this cfg)
    ...
    # SAR (raw area) fits on A-only at f=0
    ...
    # SAR_eff fits on AB (effective supported areas) at f=0
    eff = sar_fit_eff_AB(rng, A, TL, preylist;
                         n1=n1, n2=n2, Emin_total=cfg.Emin_total, Epatch=cfg.Epatch,
                         area_fracs=cfg.sar_area_fracs, samples=max(10, cfg.sar_samples ÷ 2), # keep it affordable
                         up=up, down=down, left=left, right=right,
                         row=row, col=col,
                         P=buf.P,
    ...
    # Outputs per geometry, per f
    SA_tot   = zeros(Float64, G, T)
    SAB_tot  = zeros(Float64, G, T)
    SA_pat   = zeros(Float64, G, T)
    SAB_pat  = zeros(Float64, G, T)

    # Effective areas (whole landscape after loss), per geometry
    Aeff_tot = zeros(Float64, G, T) # consumer footprint fraction
    Aeff_lcc = zeros(Float64, G, T) # LCC area fraction of footprint

    # Mechanisms
    phi      = zeros(Float64, G, T) # mean AB/A among consumers
    lccFrac  = zeros(Float64, G, T) # LCC fraction of consumer footprint
    fragfail = zeros(Float64, G, T) # consumers: total survives but patch fails
    ...
    return (SA_tot=SA_tot, SAB_tot=SAB_tot, SA_pat=SA_pat, SAB_pat=SAB_pat,
            Aeff_tot=Aeff_tot, Aeff_lcc=Aeff_lcc, phi=phi, lcc=lccFrac, fragfail=fragfail,
            fits=fits, base=base)
end

# ============================================================
# Multi-replicate aggregation (threaded)
# ============================================================
function run_many(cfg::RunConfiguration; seed::Int, reps::Int)
    ...
    # Fits/baselines (average across reps; we mainly need them for plotting predictions)
    fits_acc = [zeros(Float64, 10) for _ in 1:nthreads()] # cT zT cP zP cEffTot zEffTot cEffLCC zEffLCC (r2 ignored for avg)
    ...
    @threads for rep in 1:reps
        ...
        # only accumulate fits if finite
        f = out.fits
    ...
    fitpack = (cT=fits[1], zT=fits[2], cP=fits[3], zP=fits[4],
               ...
               r2T=fits[9], r2P=fits[10])
    basepack = (S0A_tot=base[1], S0AB_tot=base[2], S0A_pat=base[3], S0AB_pat=base[4])

    return (SA_tot=SA_tot, SAB_tot=SAB_tot, SA_pat=SA_pat, SAB_pat=SAB_pat,
            Aeff_tot=Aeff_tot, Aeff_lcc=Aeff_lcc, phi=phi, lcc=lcc, fragfail=ff,
            fits=fitpack, base=basepack)
end

# ============================================================
# Predictions for plotting: SAR raw (area-only), SAR_eff (support-aware)
# ============================================================
function predictions(cfg::RunConfiguration, agg)
    ...
    # Raw SAR: predict richness from h using A-only fits
    Ssar_tot = zeros(Float64, T)
    Ssar_pat = zeros(Float64, T)
    if isfinite(agg.fits.cT) && isfinite(agg.fits.zT)
        Ssar_tot .= agg.fits.cT .* (h .^ agg.fits.zT)
    else
        Ssar_tot .= NaN
    end
    if isfinite(agg.fits.cP) && isfinite(agg.fits.zP)
        Ssar_pat .= agg.fits.cP .* (h .^ agg.fits.zP)
    else
        Ssar_pat .= NaN
    end

    # SAR_eff: predict AB richness from effective areas (geometry-specific)
    Seff_tot = zeros(Float64, G, T)
    Seff_pat = zeros(Float64, G, T)
    ...
    # Clamp predictions to plausible ranges (avoid ugly plot spikes)
...
# ============================================================
# FIGURE 1: Short-paper main figure (absolute richness + bridge test)
# ============================================================
function fig_main(cfg::RunConfiguration, agg)
    ...
    # --- Mechanism row (compact): φ and supported-LCC ---
    for (j,g) in enumerate(cfg.geoms)
        axL = Axis(fig[3,j],
            xlabel="habitat loss f",
    ...
    Label(fig[0,1:3],
          "Static synthetic test: SAR misses losses when trophically supported habitat fragments.  SAR_eff (support-aware) is a mechanistic bridge.",
          fontsize=16)
...
# ============================================================
# FIGURE 2: Mechanistic link scatter (SAR error vs supported-LCC) + correction effect
# ============================================================
function fig_mech_scatter(cfg::RunConfiguration, agg)
    pred = predictions(cfg, agg)
    G = length(cfg.geoms)
    ...
    ax = Axis(fig[1,1:3],
        ...
        title="Fragmentation of supported habitat organizes prediction error"
    )

    # Compute losses relative to each baseline (consistent with earlier) but now compare *prediction errors*:
    # error_raw = (S0AB - SAB_obs) - (S0A - SAR_pred)  [raw SAR]
    # error_eff = (S0AB - SAB_obs) - (S0AB - SAR_eff_pred) = SAR_eff_pred - SAB_obs  [simpler]
    for (j,g) in enumerate(cfg.geoms)
        x = vec(agg.lcc[j,:])

        # Observed AB patch losses (r=0)
        Lobs = agg.base.S0AB_pat .- vec(agg.SAB_pat[j,:])

        # Raw SAR patch predicted losses (A-only SAR)
        Lraw = agg.base.S0A_pat .- pred.Ssar_pat

        # Effective SAR patch predicted losses (support LCC)
        Leff = agg.base.S0AB_pat .- vec(pred.Seff_pat[j,:])

        err_raw = Lobs .- Lraw
        err_eff = Lobs .- Leff  # should shrink if correction helps

        scatter!(ax, x, err_raw; markersize=10, label="$(String(g)) raw SAR")
        scatter!(ax, x, err_eff; markersize=10, marker=:utriangle, label="$(String(g)) SAR_eff")
        lines!(ax, x, err_raw; linewidth=2)
        lines!(ax, x, err_eff; linewidth=2, linestyle=:dash)
    end
...
# ============================================================
# LIGHT RUNNER for heatmaps (no per-species patch BFS)
# Purpose: smooth, interpretable parameter surfaces.
# Metrics:
#   - f_collapse: first f where supported-LCC < LCC_THRESH
#   - peak_raw_error_total: max over f of (AB total losses - raw SAR total losses)
# ============================================================
function run_many_light(cfg::RunConfiguration; seed::Int, reps::Int)
    println("\nRunning LIGHT reps=$reps (threads=$(nthreads())) ...")
    n1,n2 = cfg.n1, cfg.n2
    up, down, left, right = build_neighbors4(n1,n2) # used for footprint LCC only
    row, col = make_rowcol(n1,n2)
    N = n1*n2
    ...
    # thread-local buffers (reuse but ignore maxpatch arrays)
    tbufs = [make_thread_buffers(cfg) for _ in 1:nthreads()]
    ...
    lcc_sum      = [zeros(Float64, G, T) for _ in 1:nthreads()]
    Aeff_tot_sum = [zeros(Float64, G, T) for _ in 1:nthreads()]
    fits_acc     = [zeros(Float64, 2) for _ in 1:nthreads()]  # cT,zT only
    ...
    @threads for rep in 1:reps
        ...
        for (gj,g) in enumerate(cfg.geoms)
            ...
            for (t,f) in enumerate(cfg.fgrid)
                ...
                footprint = length(buf.cons_idx)
                Aeff_tot_sum[tid][gj,t] += footprint / N
                if footprint == 0
    ...
    return (SA_tot=SA_tot, SAB_tot=SAB_tot, lcc=lcc, Aeff_tot=Aeff_tot,
            cT=fits[1], zT=fits[2], S0A_tot=base[1], S0AB_tot=base[2])
end

# ============================================================
# Heatmaps (Sweep 1): denser grid + smooth metrics
# ============================================================
function fig_heatmaps_sweep1()
    cfg = RunConfiguration(
        ...
        0.45,      # niche_cut
        1.00,      # match_sigma
        3,         # k_prey (overwritten in loop)
        0.60,      # select_sigma (overwritten)
        70,        # Emin_total (scale with grid)
        70,        # Epatch (unused in light)
        FGRID_SWEEP,
    ...
    # metrics[geom, ik, is]
    fcollapse = fill(NaN, G, nk, ns)
    peak_err  = fill(NaN, G, nk, ns)

    println("\n=== SWEEP1 HEATMAPS (LIGHT): k_prey × select_sigma  (reps=$SWEEP_REPS) ===")
    ...
    @threads for sidx in 1:length(settings)
        ...
        cfg2 = RunConfiguration(cfg.n1, cfg.n2, cfg.S, cfg.basal_frac, cfg.Lmax,
                         ...
                         K_LIST[ik], SEL_LIST[is],
                         cfg.Emin_total, cfg.Epatch,
                         cfg.fgrid, cfg.geoms,
                         cfg.sar_area_fracs, cfg.sar_samples)

        out = run_many_light(cfg2; seed=SWEEP_SEED + 1000*ik + is, reps=SWEEP_REPS)

        # raw SAR prediction (total) from A-only fits
        Ssar = out.cT .* ((1.0 .- cfg2.fgrid) .^ out.zT)
        Ssar = clamp.(Ssar, 0.0, out.S0A_tot)
        Lraw = out.S0A_tot .- Ssar

        for gj in 1:G
            # collapse point: first f where supported-LCC < threshold
            tcol = findfirst(t -> out.lcc[gj,t] < LCC_THRESH, 1:length(cfg2.fgrid))
            fcollapse[gj,ik,is] = isnothing(tcol) ? cfg2.fgrid[end] : cfg2.fgrid[tcol]

            # peak raw error in losses (AB total losses - raw SAR losses)
    ...
    for (gj,g) in enumerate(cfg.geoms)
        ax1 = Axis(fig[1,gj],
            title="collapse f*: supported-LCC < $(LCC_THRESH)  — geometry=$(String(g))",
            xlabel="select_sigma", ylabel="k_prey"
        )
        hm1 = heatmap!(ax1, 1:ns, 1:nk, fcollapse[gj,:,:])
        ax1.xticks = (1:ns, string.(SEL_LIST))
        ax1.yticks = (1:nk, string.(K_LIST))
        Colorbar(fig[1,gj+3], hm1)

        ax2 = Axis(fig[2,gj],
            title="peak raw SAR error (total losses) — geometry=$(String(g))",
            xlabel="select_sigma", ylabel="k_prey"
        )
        hm2 = heatmap!(ax2, 1:ns, 1:nk, peak_err[gj,:,:])
    ...
    Label(fig[0,1:6],
        "Sweep1 (light, smooth): vary redundancy/synchrony proxies without changing rules.  Collapse f* summarizes support-connectivity fragility.",
        fontsize=16)
...
# ============================================================
# OPTIONAL: movement sanity check (radius-r connectivity in patch criterion)
# Not a novelty claim — just confirms expected qualitative sensitivity.
# ============================================================
function fig_movement_sanity(cfg::RunConfiguration; seed::Int, reps::Int, rconn::Int, geoms::Vector{Symbol})
    println("\n=== Movement sanity check: patch connectivity radius r=$rconn (reps=$reps) ===")
    cfg2 = RunConfiguration(cfg.n1, cfg.n2, cfg.S, cfg.basal_frac, cfg.Lmax,
                     cfg.niche_sigma, cfg.niche_cut, cfg.match_sigma,
                     cfg.k_prey, cfg.select_sigma, cfg.Emin_total, cfg.Epatch,
...
function main()
    ...
    println("==========================================================")
    println("BioLetters-oriented static synthetic pipeline (vNext)")
    println("grid=$(cfg.n1)x$(cfg.n2) N=$(cfg.n1*cfg.n2)   S=$(cfg.S)")
    println("threads=$(nthreads())")
    println("==========================================================")

    # FULL run -> main figures
    agg = run_many(cfg; seed=MAIN_SEED, reps=MAIN_REPS)
    fig_main(cfg, agg)
    fig_mech_scatter(cfg, agg)

    # HEATMAPS (light, smoother)
    fig_heatmaps_sweep1()

    # Movement sanity check (optional, low reps, limited geoms)
    if DO_MOVEMENT_SANITY
        fig_movement_sanity(cfg; seed=MAIN_SEED+999, reps=MOVEMENT_REPS, rconn=MOVEMENT_R, geoms=MOVEMENT_GEOMS)
    end
