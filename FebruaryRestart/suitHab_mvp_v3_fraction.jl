# suitHab_mvp_v3_fraction.jl
#
# Synthetic MVP v3:
#   - Fractional trophic support rule:
#       prey_fraction >= q_min
#   - Twin-axis per geometry: EA/EAB (left) + dE* (right)
#   - Realized prey diversity (Dprey) heatmaps to validate synchrony knob
#   - Prints summary: dE*(0.5), AUC(dE*), Dprey_mean
...
# ----------------------------
# Extinction counts (Emin) and fractional B-rule
# ----------------------------
...
"""
Fraction rule:
  sup = ( (# prey present) / m ) >= q_min
"""
function extinction_count_AB_fraction(A::BitMatrix, keep::BitVector,
                                     TL::Vector{Int}, preylist::Vector{Vector{Int}};
                                     Emin::Int=1, q_min::Float64=0.5)
...
"""
function run_one(rng::AbstractRNG, p::RunParams1, fgrid::Vector{Float64})
    ...
    EAB0 = extinction_count_AB_fraction(A, keep0, TL, preylist; Emin=p.Emin, q_min=p.q_min)

    for (t, f) in enumerate(fgrid)
        keep = keep_from_order(ord, f, N)
        EA[t]  = extinction_count_Aonly(A, keep; Emin=p.Emin)
        EAB[t] = extinction_count_AB_fraction(A, keep, TL, preylist; Emin=p.Emin, q_min=p.q_min)
...
function run_sweep(; seed::Int=1234, reps::Int=40)
    ...
    # Defaults tuned for earlier action; q_min controls trophic strictness
    base = RunParams1(
        80, 80,
        80,
        0.25,
        ...
        :random,
        50,     # Emin
        0.5     # q_min (fraction of prey required)
    )
    ...
    for g in geoms, kp in kprey_list, ss in selects_list
        p = RunParams1(base.n1, base.n2, base.S, base.basal_frac, base.Lmax,
                      base.niche_sigma, base.niche_cut,
                      kp, ss, base.match_sigma, g, base.Emin, base.q_min)
    ...
    # ----------------------------
    # Plot: row 1 twin-axis panels; row 2 Dprey heatmaps
    # ----------------------------
    ...
    for (j, g) in enumerate(geoms)
        axL = Axis(fig[1, j],
            ...
            ylabel = (j == 1 ? "EA / EAB" : ""),
            title  = "geometry = $(String(g))  (q_min=$(round(base.q_min, digits=2)))"
        )
        axR = Axis(fig[1, j],
    ...
    @assert !isempty(Dvals) "All Dprey values are NaN."
    ...
    for g in geoms
        ...
        # optional but recommended: mark undefined cells
