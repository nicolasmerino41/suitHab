...
- **BSH** (Biotically Safe Habitat) is the fraction of grid cells that are **both** climatically suitable and biotically supported:
\[
\boxed{\text{BSH}_s = A_s\,p_s.}
\]
...
\[
\Delta B_s = A_1 p_1 - A_0 p_0
\]
is the change in BSH for species \(s\).

## Linear decomposition (why synergy was \(\approx 0\))

Define midpoints \(\bar A = (A_0{+}A_1)/2\) and \(\bar p = (p_0{+}p_1)/2\). The symmetric (Shapley-like) decomposition for the *linear* outcome \(B=Ap\) is
\[
\boxed{\Delta B_s
...
Because \(B=Ap\) is bilinear, the midpoint residual (synergy) is exactly 0 (up to numerical noise). In random loss, \(\Delta p\approx 0\) and almost all change is climate-only. When loss targets prey-rich suitable cells (hotspot), \(\Delta p<0\) and the interaction-only term becomes negative.

## Nonlinear viability (making synergy real)

Many management questions care about *viability* rather than raw area. We therefore define a **nonlinear outcome**
\[
\boxed{U(A,p) \;=\; A\,\phi(p)}
\]
...
\[
\begin{aligned}
\Delta U_s &= U(A_1,p_1) - U(A_0,p_0),\\\\
\text{climate-only} &= U(A_1,\bar p) - U(A_0,\bar p),\\\\
\text{interaction-only} &= U(\bar A,p_1) - U(\bar A,p_0),\\\\
\text{synergy} &= \Delta U_s - (\text{climate-only}) - (\text{interaction-only}).
\end{aligned}
\]
...
- With \(B=Ap\), synergy vanishes by construction; interactions still matter via \(\Delta p\).
- With a **viability** outcome \(U(A,p)\), synergy becomes a genuine, interpretable quantity.
- Comparing **random / clustered / hotspot** loss trajectories for \((\text{climate}, \text{interaction}, \text{synergy})\) reveals when and how trophic structure amplifies the damage beyond climate loss alone.
